...
export const SharedLinksView: React.FC<SharedLinksViewProps> = ({
  ...
  return (
    <div className="flex flex-col space-y-6 pb-20 pt-20 md:pt-20 animate-fadeIn">
      ...
      <div className="fixed top-0 left-0 right-0 md:left-64 z-40 bg-[#FDFBF7] border-b-2 border-stone-800 shadow-sm px-4 md:px-8 pt-3 pb-2 space-y-2">
        <div className="flex items-center justify-between gap-2 max-w-7xl mx-auto w-full">
          <div className="relative flex-1 min-w-0">
            ...
            <input
              ...
              className="w-full bg-white border-2 border-stone-800 rounded-xl pl-9 pr-3 py-1.5 text-xs font-medium outline-none shadow-[2px_2px_0px_0px_#1c1917]"
            />
          </div>
          <div className="flex items-center gap-1.5 shrink-0 relative">
            <button
              ...
              className="p-1.5 bg-white hover:bg-stone-100 text-stone-700 rounded-xl border-2 border-stone-800 shadow-[2px_2px_0px_0px_#1c1917] active:translate-x-0.5 active:translate-y-0.5 transition-all cursor-pointer"
            >
            ...
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white border-3 border-stone-800 rounded-2xl shadow-[4px_4px_0px_0px_#1c1917] py-2 z-50 animate-fadeIn">
                  <div className="px-3 py-1.5 text-[11px] font-extrabold text-stone-400 uppercase tracking-wider border-b border-stone-200 mb-1">
                    Options de tri
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className={`w-full text-left px-4 py-2 text-xs font-bold flex items-center gap-2 hover:bg-orange-50 transition-colors ${
                      sortBy === 'size' ? 'text-orange-600 bg-orange-50/50' : 'text-stone-800'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-xs font-bold flex items-center gap-2 hover:bg-orange-50 transition-colors ${
                      sortBy === 'recent' ? 'text-orange-600 bg-orange-50/50' : 'text-stone-800'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-xs font-bold flex items-center gap-2 hover:bg-orange-50 transition-colors ${
                      sortBy === 'oldest' ? 'text-orange-600 bg-orange-50/50' : 'text-stone-800'
                    }`}
                  >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pt-0.5 pb-0.5 max-w-7xl mx-auto w-full">
          <button
            ...
            className={`px-3 py-1 rounded-xl border-2 border-stone-800 text-xs font-bold shadow-[2px_2px_0px_0px_#1c1917] flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap ${
              filterStatus === 'all' ? 'bg-orange-500 text-white' : 'bg-white text-stone-800 hover:bg-stone-100'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 rounded-xl border-2 border-stone-800 text-xs font-bold shadow-[2px_2px_0px_0px_#1c1917] flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap ${
              filterStatus === 'unpublished' ? 'bg-amber-500 text-white' : 'bg-white text-stone-800 hover:bg-stone-100'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 rounded-xl border-2 border-stone-800 text-xs font-bold shadow-[2px_2px_0px_0px_#1c1917] flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap ${
              filterStatus === 'published' ? 'bg-emerald-500 text-white' : 'bg-white text-stone-800 hover:bg-stone-100'
            }`}
          >

...
const COLORS = [
  { name: 'Vert Pastel', bg: 'bg-emerald-100', text: 'text-emerald-900', border: 'border-emerald-300' },
  { name: 'Bleu Pastel', bg: 'bg-sky-100', text: 'text-sky-900', border: 'border-sky-300' },
  { name: 'Jaune Pastel', bg: 'bg-amber-100', text: 'text-amber-900', border: 'border-amber-300' },
  { name: 'Rose Pastel', bg: 'bg-rose-100', text: 'text-rose-900', border: 'border-rose-300' },
  { name: 'Violet Pastel', bg: 'bg-purple-100', text: 'text-purple-900', border: 'border-purple-300' },
];
  ...
  return (
    <div className="absolute inset-x-0 md:left-64 md:right-0 bottom-16 md:bottom-0 top-[56px] md:top-[60px] z-30 w-full md:w-[calc(100%-16rem)] bg-[#FDFBF7] text-stone-900 flex flex-col overflow-hidden">
      ...
      <div className="flex-none px-3 py-2 bg-[#FDFBF7]/95 backdrop-blur-md border-b border-stone-200 flex items-center justify-between z-40 shadow-sm">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 px-2.5 py-1 bg-[#E8DFD0] hover:bg-[#D4C9B5] text-[#2D4A3E] font-bold text-xs rounded-lg border-2 border-[#2D4A3E] shadow-[1px_1px_0px_0px_#1c1917] transition-all cursor-pointer active:translate-x-0.5 active:translate-y-0.5"
        >
          <ArrowLeft className="w-3.5 h-3.5" />
          <span>Retour</span>
        </button>
        <div className="text-center flex-1 mx-2">
          <h1 className="text-base sm:text-lg font-serif font-bold text-[#2D4A3E] truncate">Mon emploi du temps</h1>
        </div>
        <button
          onClick={addHourRow}
          className="flex items-center gap-1 px-2.5 py-1 bg-[#2D4A3E] hover:bg-[#20342b] text-white font-bold text-xs rounded-lg shadow transition-all cursor-pointer whitespace-nowrap"
        >
      ...
      >
        <div 
          ref={tableRef}
          className="min-w-[900px] max-w-full mx-auto bg-white rounded-xl shadow-lg border-2 border-stone-800 overflow-hidden"
          style={{ zoom: `${zoomLevel}%` }}
        >
          {/* Header title inside the table area */}
          <div className="bg-[#F5F0E8] py-3 px-4 border-b-2 border-stone-800 flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <Calendar className="w-5 h-5 text-[#2D4A3E]" />
                <span className="font-serif font-bold text-base text-[#2D4A3E]">Emploi du temps hebdomadaire</span>
              </div>
              {/* Zoom Controls */}
              <div className="flex items-center gap-1 bg-white border border-stone-300 rounded-lg p-1 shadow-xs">
                <button
                  type="button"
                ...
                <button
                  ...
                  className="px-2 py-0.5 text-xs font-bold text-[#2D4A3E] hover:bg-stone-100 rounded transition-colors cursor-pointer"
                >
                  {zoomLevel}%
                </button>
                <button
            ...
            <span className="text-xs text-stone-600 bg-stone-200/70 px-2.5 py-1 rounded-full font-medium">Glissez horizontalement et verticalement • Cliquez sur les jours et heures pour les renommer</span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-xs sm:text-sm table-fixed">
            <thead>
              <tr className="bg-[#E8DFD0] text-[#2D4A3E] border-b-2 border-stone-800">
                {/* Diagonal Split Top-Left Corner Box */}
                <th className="relative border-r-2 border-stone-800 w-32 h-16 p-0 overflow-hidden bg-[#E8DFD0]">
                  <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="none" viewBox="0 0 100 100">
                    <line x1="0" y1="0" x2="100" y2="100" stroke="#1c1917" strokeWidth="2" />
                  </svg>
                  <div className="absolute top-1 right-2 text-[11px] font-bold uppercase tracking-wider text-[#2D4A3E]">
                    Jours
                  </div>
                  <div className="absolute bottom-1 left-2 text-[11px] font-bold uppercase tracking-wider text-[#2D4A3E]">
                    Heures
                  </div>
                </th>
                {days.map((day, dIdx) => (
                  <th
                    ...
                    className="py-3 px-3 font-bold border-r border-stone-800 text-center uppercase tracking-wider last:border-r-0 cursor-pointer hover:bg-[#dfd4c0] transition-colors group relative"
                    title="Cliquer pour modifier ce jour"
                  >
                    <div className="flex items-center justify-center gap-1">
                      <span>{day}</span>
                      <Edit2 className="w-3 h-3 opacity-0 group-hover:opacity-100 text-[#2D4A3E] transition-opacity" />
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {hours.map((hour, hIdx) => (
                <tr key={`${hour}-${hIdx}`} className="border-b border-stone-300 hover:bg-stone-50/50 transition-colors">
                  <td
                    onClick={() => handleHeaderClick('hour', hIdx)}
                    className="py-3 px-2 font-bold text-stone-800 bg-[#F9F6F0] border-r-2 border-stone-800 text-center whitespace-nowrap cursor-pointer hover:bg-[#f0e8dc] transition-colors group"
                    title="Cliquer pour modifier cette heure"
                  >
                    <div className="flex items-center justify-center gap-1">
                      <span>{hour}</span>
                      <Edit2 className="w-3 h-3 opacity-0 group-hover:opacity-100 text-stone-600 transition-opacity" />
                    </div>
                  </td>
                  {days.map((day) => {
                    const key = `${day}_${hour}`;
                    const entry = scheduleData[key];
                    const colorStyle = COLORS.find(c => c.bg === entry?.color) || COLORS[0];

                    return (
                      <td
                        key={day}
                        onClick={() => openSlot(day, hour)}
                        className={`p-2 border-r border-stone-300 last:border-r-0 cursor-pointer transition-all hover:bg-stone-100 relative group align-top ${
                          entry ? colorStyle.bg + ' ' + colorStyle.border + ' border-l-4' : 'h-16'
                        }`}
                      >
                        {entry ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center opacity-0 group-hover:opacity-40 text-stone-400">
      ...
      {editingHeader && (
        <div className="fixed inset-0 z-50 bg-stone-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-7 shadow-2xl border-2 border-stone-900 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-5 pb-3 border-b-2 border-stone-200">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 border-2 border-stone-400 rounded-xl focus:border-[#2D4A3E] focus:outline-none text-sm font-bold text-stone-900 bg-stone-50"
                  autoFocus
                  onKeyDown={(e) => {
              ...
              <div className="flex items-center justify-end gap-3 pt-3">
                <button
                  type="button"
                  onClick={() => setEditingHeader(null)}
                  className="px-4 py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-800 font-bold text-xs rounded-xl transition-all cursor-pointer"
                >
                  Annuler
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-[#2D4A3E] hover:bg-[#20342b] text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer"
                >
                  Enregistrer
                </button>
              </div>
                ...
                <div className="pt-4 border-t-2 border-stone-200 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="px-3 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border-2 border-rose-300 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-xs"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-800 border-2 border-stone-300 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-xs"
                    >
                      <Plus className="w-4 h-4" />
        ...
        <div className="fixed inset-0 z-60 bg-stone-900/70 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border-2 border-stone-900 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-[#2D4A3E]/10 flex items-center justify-center text-[#2D4A3E] shrink-0">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-stone-200">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#2D4A3E] hover:bg-[#20342b] text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer"
              >
                Ajouter
              </button>
            </div>
      ...
      {activeSlot && (
        <div className="fixed inset-0 z-50 bg-stone-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border-2 border-stone-900 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-4 pb-3 border-b-2 border-stone-200">
              <div className="flex items-center gap-2">
                  ...
                  {COLORS.map((c) => (
                    <button
                      ...
                      className={`w-8 h-8 rounded-full ${c.bg} border-2 ${
                        selectedColor === c.bg ? 'border-stone-900 scale-110 shadow-md' : 'border-stone-300'
                      } transition-all cursor-pointer flex items-center justify-center`}
                    >
                      {selectedColor === c.bg && <Check className="w-4 h-4 text-stone-900" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t-2 border-stone-200">
                {scheduleData[`${activeSlot.day}_${activeSlot.hour}`] && (
                  <button
                    ...
                    className="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs rounded-xl border border-rose-300 transition-all flex items-center gap-1.5 cursor-pointer mr-auto"
                  >
                ...
                <button
                  type="button"
                  onClick={saveEntry}
                  className="px-5 py-2 bg-[#2D4A3E] hover:bg-[#20342b] text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer"
                >
                  Enregistrer
                </button>
              </div>

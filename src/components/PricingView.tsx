...
export const PricingView: React.FC<PricingViewProps> = ({ onBack, onSelectPlan }) => {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 top-[48px] md:top-[52px] md:left-64 z-30 w-full md:w-[calc(100%-16rem)] min-h-screen bg-[#F5F0E8] text-[#2D4A3E] overflow-y-auto animate-fadeIn pb-24">
      {/* Fixed floating buttons directly below the top header */}
      <div className="fixed inset-x-0 top-[48px] md:top-[52px] z-40 px-4 sm:px-8 py-2 flex items-center justify-between w-full pointer-events-none">
        <div className="w-full max-w-[1250px] mx-auto flex items-center justify-between pointer-events-auto">
          <button
            onClick={onBack}
          ...
          <div className="bg-[#E8DFD0] rounded-full p-1 flex items-center shadow-xs border-2 border-[#1c1917]">
            <button
              ...
              className={`flex items-center gap-1 px-3.5 py-1 rounded-full text-xs font-bold transition-all cursor-pointer ${
                billingCycle === 'annual'
                  ? 'bg-[#F5F0E8] text-[#2D4A3E] shadow-xs'
                  : 'bg-transparent text-[#5C6B5A] hover:text-[#2D4A3E]'
              }`}
            >
              <span>Ans</span>
              <span className="bg-[#2D4A3E] text-[#F5F0E8] text-[9px] font-bold uppercase px-1.5 py-0.5 rounded">
                -10%
              </span>
            </button>
            <button
      ...
      <div className="w-full max-w-[1250px] mx-auto px-4 pt-16">
        {/* Header Section */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch pb-20 w-full max-w-[1250px] mx-auto px-2">
          {/* Basic Card */}
          <div className="bg-[#E8DFD0] rounded-2xl p-6 sm:p-7 flex flex-col justify-between border border-[#D4C9B5] relative max-w-[380px] w-full mx-auto shadow-sm">
            <div>
              <h3 className="text-2xl sm:text-3xl font-serif font-normal text-[#2D4A3E] mb-2 sm:mb-3">Basique</h3>
              <p className="text-xs sm:text-sm font-sans text-[#5C6B5A] mb-4 sm:mb-6 leading-relaxed">
                Pour les particuliers et petites équipes qui débutent.
              </p>
              <div className="flex items-baseline mb-4 sm:mb-6">
                <span className="text-5xl sm:text-6xl font-serif font-normal text-[#2D4A3E]">
                  ${billingCycle === 'annual' ? '9' : '10'}
                </span>
                <span className="text-lg sm:text-xl font-sans text-[#2D4A3E] ml-1">{billingCycle === 'annual' ? '/an' : '/mois'}</span>
              </div>
              <div className="h-0.5 w-full bg-gradient-to-r from-[#C9B896] to-[#D4C9B5] mb-4 sm:mb-6"></div>
              <p className="text-xs sm:text-sm font-semibold text-[#2D4A3E] mb-3 sm:mb-4">Ce qui est inclus :</p>
              <ul className="space-y-2.5 sm:space-y-3 mb-6 sm:mb-8">
                {[
                  ...
                  <li key={i} className="flex items-start gap-2.5 sm:gap-3">
                    <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-[#2D4A3E] flex items-center justify-center shrink-0 mt-0.5">
                      <Check className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#F5F0E8]" />
                    </div>
                    <span className="text-xs sm:text-sm text-[#2D4A3E]">{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {/* Pro Card */}
          <div className="bg-[#2D4A3E] rounded-2xl p-6 sm:p-7 flex flex-col justify-between border-2 border-[#2D4A3E] relative max-w-[380px] w-full mx-auto shadow-md">
            <div>
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-2xl sm:text-3xl font-serif font-normal text-[#F5F0E8]">Pro</h3>
                <span className="bg-[#C9B896] text-[#2D4A3E] text-xs font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                  Populaire
                </span>
              </div>
          ...
          <div className="bg-[#E8DFD0] rounded-2xl p-6 sm:p-7 flex flex-col justify-between border border-[#D4C9B5] relative max-w-[380px] w-full mx-auto shadow-sm">
            <div>
              <h3 className="text-2xl sm:text-3xl font-serif font-normal text-[#2D4A3E] mb-2 sm:mb-3">Entreprise</h3>
              <p className="text-xs sm:text-sm font-sans text-[#5C6B5A] mb-4 sm:mb-6 leading-relaxed">
              ...
              <div className="h-0.5 w-full bg-gradient-to-r from-[#C9B896] to-[#D4C9B5] mb-4 sm:mb-6"></div>
              <p className="text-xs sm:text-sm font-semibold text-[#2D4A3E] mb-3 sm:mb-4">Tout dans Pro, plus :</p>
              <ul className="space-y-2.5 sm:space-y-3 mb-6 sm:mb-8">
                {[

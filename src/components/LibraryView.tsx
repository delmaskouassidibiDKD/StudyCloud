...
export const LibraryView: React.FC<LibraryViewProps> = ({
  ...
  const [downloadMenuTarget, setDownloadMenuTarget] = useState<{ doc: any; rect: DOMRect } | null>(null);

  const handleOpenDownloadMenu = (e: React.MouseEvent, doc: any) => {
    e.stopPropagation();
    if (downloadMenuTarget?.doc.id === doc.id) {
      setDownloadMenuTarget(null);
    } else {
      const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
      setDownloadMenuTarget({ doc, rect });
    }
  };

  // Positionne le menu déroulant de manière verticale vers le bas par rapport au bouton
  const getDownloadMenuStyle = (rect: DOMRect): React.CSSProperties => {
    ...
    // De manière VERTICALE VERS LE BAS
    const spaceBelow = window.innerHeight - rect.bottom;
    const menuHeight = 180;
    let top = rect.bottom + margin;
    if (spaceBelow < menuHeight && rect.top > menuHeight + 20) {
  ...
  return (
    <div className="flex flex-col space-y-4 pb-24 pt-24 md:pt-26 animate-fadeIn w-full px-2 sm:px-4">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-1.5 border-t border-white/20 gap-1.5">
                          ...
                          {doc.file_url ? (
                            <button
                              type="button"
                              onClick={(e) => handleOpenDownloadMenu(e, doc)}
      ...
      {/* Menu déroulant de téléchargement vertical vers le bas (Design néo-brutaliste StudyCloud, zéro débordement) */}
      {downloadMenuTarget && (
        <>
          {/* Fond sombre transparent pour fermer au clic extérieur */}
          <div
            className="fixed inset-0 z-[99998] bg-black/30 backdrop-blur-[2px] animate-fadeIn"
            onClick={(e) => {
              e.stopPropagation();
              setDownloadMenuTarget(null);
            }}
          ...
          <div
            style={getDownloadMenuStyle(downloadMenuTarget.rect)}
            className="bg-[#FDFBF7] dark:bg-stone-900 border-3 border-stone-800 dark:border-stone-700 rounded-2xl shadow-[6px_6px_0px_0px_#1c1917] p-3 text-left animate-in fade-in zoom-in-95 duration-150 select-none overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* En-tête : Titre du fichier + bouton croix pour fermer */}
            <div className="flex items-center justify-between pb-2 mb-2.5 border-b-2 border-stone-800/20 dark:border-stone-700">
              <div className="flex items-center gap-2 min-w-0 pr-2">
                <div className="w-7 h-7 rounded-lg bg-orange-500 text-white flex items-center justify-center shrink-0 border border-stone-800 shadow-[1.5px_1.5px_0px_0px_#1c1917]">
                  <Download className="w-4 h-4 stroke-[2.5]" />
                </div>
                <div className="min-w-0">
                  <h4 className="text-xs font-black text-stone-900 dark:text-white tracking-tight leading-tight">
                    Téléchargement
                  </h4>
                  <p className="text-[10px] font-bold text-stone-600 dark:text-stone-300 truncate max-w-[190px]">
                    {downloadMenuTarget.doc.title || downloadMenuTarget.doc.file_name || 'Document'}
                  </p>
                </div>
              </div>
              <button
                ...
                className="p-1 text-stone-500 hover:text-stone-900 dark:hover:text-white rounded-lg hover:bg-stone-200 dark:hover:bg-stone-800 border border-stone-800 transition-all cursor-pointer shadow-[1px_1px_0px_0px_#1c1917] active:scale-95"
                title="Fermer"
              >
            ...
            {/* Options verticales ordonnées vers le bas */}
            <div className="flex flex-col gap-2">
              {/* Option 1 : Télécharger dans l'appareil */}
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  const doc = downloadMenuTarget.doc;
                  setDownloadMenuTarget(null);
                  handleDownloadToDevice(doc);
                }}
                className="w-full p-2.5 rounded-xl bg-white hover:bg-orange-50 active:bg-orange-100 dark:bg-stone-800 dark:hover:bg-stone-700/80 border-2 border-stone-800 shadow-[2.5px_2.5px_0px_0px_#1c1917] hover:shadow-[3.5px_3.5px_0px_0px_#1c1917] active:translate-x-0.5 active:translate-y-0.5 transition-all flex items-center gap-3 text-left cursor-pointer group"
              >
                <div className="w-9 h-9 rounded-xl bg-orange-500 text-white flex items-center justify-center shrink-0 border border-stone-800 shadow-[1.5px_1.5px_0px_0px_#1c1917] group-hover:scale-105 transition-transform">
                  <MonitorDown className="w-5 h-5 stroke-[2.2]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-black text-stone-900 dark:text-white leading-tight">
                    Télécharger dans l'appareil
                  </p>
                  <p className="text-[10px] text-stone-500 dark:text-stone-400 font-medium">
                    Fichier direct sur votre PC ou téléphone
                  </p>
                </div>
              </button>

              {/* Option 2 : Télécharger dans StudyCloud (Mes fichiers) */}
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  const doc = downloadMenuTarget.doc;
                  setDownloadMenuTarget(null);
                  handleSaveToStudyCloud(doc);
                }}
                className="w-full p-2.5 rounded-xl bg-white hover:bg-emerald-50 active:bg-emerald-100 dark:bg-stone-800 dark:hover:bg-stone-700/80 border-2 border-stone-800 shadow-[2.5px_2.5px_0px_0px_#1c1917] hover:shadow-[3.5px_3.5px_0px_0px_#1c1917] active:translate-x-0.5 active:translate-y-0.5 transition-all flex items-center gap-3 text-left cursor-pointer group"
              >
                <div className="w-9 h-9 rounded-xl bg-emerald-600 text-white flex items-center justify-center shrink-0 border border-stone-800 shadow-[1.5px_1.5px_0px_0px_#1c1917] group-hover:scale-105 transition-transform">
                  <CloudDownload className="w-5 h-5 stroke-[2.2]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-black text-stone-900 dark:text-white leading-tight">
                    Télécharger dans StudyCloud
                  </p>
                  <p className="text-[10px] text-stone-500 dark:text-stone-400 font-medium">
                    Enregistrer dans votre espace Mes fichiers
                  </p>
                </div>
              </button>
            </div>

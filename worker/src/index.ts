...
function renderProductSharePageHtml(product: any, sellerShop: any, sellerUser: any, relatedProducts: any[], appOrigin: string): string {
  ...
  // Message WhatsApp pré-rempli pour la commande
  ...
  const whatsappUrl = cleanPhone
    ...
    : `https://wa.me/?text=${encodeURIComponent(autoMessage)}`;
...
<head>
  ...
  <style>
    * {
    ...
    /* Badge Logo StudyCloud incrusté */
    .sc-badge {
      position: absolute;
      top: 16px;
      left: 18px;
        ...
        <div class="desc-section">
          ...
          ${hasLongDesc ? `
            <button type="button" id="descBtn" onclick="toggleDescription()" class="desc-toggle-btn">
              <span id="descBtnText">Dérouler la description</span>
  ...
  <script>
    ...
    function toggleDescription() {
      ...
      if (text && btn && btnText) {
        if (isExpanded) {
          ...
          btnText.innerText = 'Réduire la description';
        } else {
          text.classList.add('desc-clamped');
          btn.classList.remove('expanded');
          btnText.innerText = 'Dérouler la description';
        }
      }
    }

    // PANIER (LOCALSTORAGE SYNCHRONISÉ AVEC L'APP)
    function getCart() {
      try {
    ...
    function handleCartToggle() {
      ...
      if (cart.includes(PRODUCT_ID)) {
        ...
        showToast('"' + PRODUCT_TITLE + '" retiré du panier');
      } else {
        cart.push(PRODUCT_ID);
        showToast('"' + PRODUCT_TITLE + '" ajouté au panier !');
      }
      try {
    ...
    function handleFollowToggle() {
      ...
      if (list.includes(SELLER_ID)) {
        ...
        showToast('Désabonné de ' + SELLER_NAME);
      } else {
        list.push(SELLER_ID);
        showToast('Vous êtes maintenant abonné à ' + SELLER_NAME + ' !');
      }
      try {
    ...
    function handleShare() {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(window.location.href).then(() => {
          showToast('Lien du produit copié dans le presse-papier !');
        }).catch(() => {
          fallbackShare();
        });
      } else {
...
<body style="margin:0;padding:0;background-color:#0f0c29;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#1e293b;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#0f0c29;padding:40px 16px;">
    <tr>
      ...
      if (path === '/api/cloud/upload' && (method === 'PUT' || method === 'POST')) {
        ...
        // 1. Détection automatique de la nature du fichier
      ...
      if (path === '/api/cloud/classeur/folders') {
        ...
        if (method === 'DELETE') {
          ...
          // Récupérer le dossier pour l'archiver dans la corbeille
          const targetFolder: any = await env.DB.prepare(`
          ...
          if (targetFolder) {
            ...
            `).bind(
              folderId, reqUserId, targetFolder.name, targetFolder.parent_id || '',
          ...
          return jsonResponse({ success: true, message: 'Dossier et son contenu déplacés dans la corbeille' }, 200, origin);
        }
      }
      ...
      if (path === '/api/cloud/downloads') {
        ...
        if (method === 'DELETE') {
          ...
          const dlFile: any = await env.DB.prepare(`
          ...
          if (dlFile) {
            await env.DB.prepare(`
              ...
              ON CONFLICT(id) DO UPDATE SET deleted_at = CURRENT_TIMESTAMP
            `).bind(
              dlFile.id, reqUserId, dlFile.name, dlFile.size, dlFile.size_bytes, dlFile.extension || '',
              JSON.stringify({ sourceUrl: dlFile.source_url, source: dlFile.source }),
              dlFile.downloaded_at || new Date().toLocaleDateString('fr-FR'),
              dlFile.r2_key, dlFile.file_url
            ).run();
          }
          ...
          if (secFile) {
            ...
            if (origCat === 'classeur_folder' || secFile.category === 'folder') {
              await env.DB.prepare(`
                ...
                ON CONFLICT(id) DO NOTHING
              `).bind(
                secFile.id, reqUserId, meta.parentId || meta.parent_id || null, secFile.name,
                meta.modelId || meta.model_id || '1', meta.primaryColor || meta.primary_color || '#EA580C',
                meta.accentColor || meta.accent_color || '#F97316', meta.iconName || meta.icon_name || 'Folder',
                meta.textDark ? 1 : 0, Number(meta.positionX || meta.position_x || 0),
                Number(meta.positionY || meta.position_y || 0), Number(meta.displayOrder || meta.display_order || 0),
                Number(meta.zoomLevel || meta.zoom_level || 10), meta.isPinned ? 1 : 0, meta.isFavorite ? 1 : 0
              ).run();
            } else if (origFolder) {
              await env.DB.prepare(`
                INSERT INTO classeur_files (
            ...
            if (item) {
              ...
              if (srcCat === 'classeur_folder' || item.category === 'folder') {
                ...
                await env.DB.prepare(`
                  ...
                  ON CONFLICT(id) DO NOTHING
                `).bind(
                  item.id, reqUserId, fMeta.parentId || fMeta.parent_id || null, item.name,
                  fMeta.modelId || fMeta.model_id || '1', fMeta.primaryColor || fMeta.primary_color || '#EA580C',
      ...
      // ----------------------------------------------------------------------
      // 11. FICHIERS & DOSSIERS ÉPINGLÉS (/api/cloud/pinned)
      // ----------------------------------------------------------------------
      ...
      // ----------------------------------------------------------------------
      // 12. RENOMMER UN FICHIER OU UN DOSSIER (/api/cloud/rename)
      // ----------------------------------------------------------------------
      if (path === '/api/cloud/rename' && (method === 'POST' || method === 'PUT')) {
        ...
        return jsonResponse({ success: true, message: 'Nom modifié avec succès', name: newName }, 200, origin);
      }

      // ----------------------------------------------------------------------
      // 13. DUPLIQUER UN FICHIER OU UN DOSSIER (/api/cloud/duplicate)
      // ----------------------------------------------------------------------
      if (path === '/api/cloud/duplicate' && method === 'POST') {
        ...
        if (category === 'classeur_folder' || category === 'folder') {
          ...
          if (!orig) return errorResponse('Dossier introuvable', 404, origin);
          ...
          await env.DB.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, CURRENT_TIMESTAMP)
          `).bind(
            newId, reqUserId, finalName, orig.model, orig.primary_color, orig.secondary_color,
            orig.text_dark, (orig.display_order || 0) + 1, (orig.position_x || 0) + 20, (orig.position_y || 0) + 20,
          ...
          const duplicatedFolder = {
            ...
            textDark: !!orig.text_dark,
            displayOrder: (orig.display_order || 0) + 1,
            positionX: (orig.position_x || 0) + 20,
            positionY: (orig.position_y || 0) + 20,
            zoomLevel: orig.zoom_level || 10,
            dateText: orig.date_text,
            isFavorite: false,
            isPinned: false,
            parentId: orig.parent_id || null,
            createdAt: Date.now()
          };
      ...
      // ----------------------------------------------------------------------
      // 14. DÉPLACER OU COPIER DES ÉLÉMENTS VERS DES DOSSIERS (/api/cloud/move)
      // ----------------------------------------------------------------------
      if (path === '/api/cloud/move' && method === 'POST') {
        ...
        const targetFolderIds: string[] = Array.isArray(body.targetFolderIds) 
          ? body.targetFolderIds 
          : (body.targetFolderId ? [body.targetFolderId] : []);
        const mode: 'move' | 'copy' = body.mode === 'copy' ? 'copy' : 'move';

        if (rawItems.length === 0) return errorResponse('Aucun élément spécifié', 400, origin);
        if (targetFolderIds.length === 0) return errorResponse('Aucun dossier récepteur sélectionné', 400, origin);
        ...
        for (const item of rawItems) {
          ...
          // Écrire dans chaque dossier récepteur sélectionné
          for (const folderId of targetFolderIds) {
            ...
            `).bind(
              newFileId, reqUserId, folderId, itemName, itemSize, itemSizeBytes, itemCat, itemExt,
              item.source || 'transfer', dateText, isNotepad, itemName, contentText, previewUrl, r2Key,
              fileUrl
            ).run();
      ...
      // ----------------------------------------------------------------------
      // Endpoint direct d'image binaire réelle du produit (JPEG/PNG)
      // Résout le problème WhatsApp qui refuse le format SVG et affiche le logo par défaut
      // ----------------------------------------------------------------------
      if (path.match(/^\/api\/products\/[^/]+\/image$/) && method === 'GET') {
        ...
        if (firstImg.startsWith('data:')) {
          ...
          // Récupération directe et ultra-rapide depuis Cloudflare R2
          try {
            let r2Key = firstImg;
  ...
  <g transform="translate(560, 485)">
    ...
    <text x="35" y="35" font-family="system-ui, sans-serif" font-size="18" font-weight="900" fill="#ffffff">💬 Commander sur WhatsApp</text>
      ...
      // ----------------------------------------------------------------------
      // Page Produit Autonome Complète (Clone identique de l'application)
      // Connectée en temps réel à D1 avec aperçu WhatsApp optimisé (JPEG réel)
      // ----------------------------------------------------------------------
      if (path.match(/^\/share\/product\/[^/]+$/) && method === 'GET') {
        ...
        // Récupérer le profil du vendeur et d'autres produits du même vendeur
        const [sellerUser, sellerShop, relatedRes]: any = await Promise.all([

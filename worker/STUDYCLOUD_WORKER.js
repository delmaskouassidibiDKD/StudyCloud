    ...
    <tr>
      ...
      if (path === "/api/app-links") {
        ...
        if (method === "POST" || method === "PUT") {
          ...
          if (!id || !linkUrl)
            return errorResponse("id et url requis", 400, origin);
          if (env.DB) {
            await env.DB.prepare(`
      ...
      if (path.startsWith("/link/")) {
        ...
        if (!targetUrl) {
          ...
          else if (linkId === "telegram")
            targetUrl = "https://t.me/+QtRhdlTsMHxjODk0";
          else if (linkId === "whatsapp")
            targetUrl = "https://chat.whatsapp.com/IPOnCB9rJhn7JECrNY20Ea";
        }
        if (targetUrl) {
          return Response.redirect(targetUrl, 302);
        }
        return errorResponse("Lien non trouv\xE9", 404, origin);
      }
      if (path === "/api/chat") {
        const userId = url.searchParams.get("userId");
